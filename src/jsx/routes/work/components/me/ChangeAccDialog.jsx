...
class ChangeAccDialog extends PureComponent{
    ...
    render(){
        ...
        var {onCancel,accName:oldname,accType,mt4Id} = this.props,
            {accName} = this.state;
        return(
            <Confirm sureText={"修改"} onSure={this.onSure} onCancel={onCancel}>
                <div>
                    <p className="font30 mg-bt-20 mg-bt-40 mg-tp-10">
                    {oldname}
                    {accType>0?<span className="font26 right">{mt4Id}</span>:null}
                    </p>
